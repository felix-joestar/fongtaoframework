...
- 业务基线表结构统一维护在业务聚合模块的 `_database_schema/` 下；starter 不携带生产 schema、初始化数据或默认账号，也不得自动执行 DDL/DML。
...
- `fongtao-starter-core` 承载响应、可扩展错误码、基础异常、分页、链路上下文、校验分组和树处理等跨模块纯 Java 契约；包按 `result`、`exception`、`page`、`trace`、`validation`、`util` 分责，且不得依赖 Web、数据库、安全、缓存或业务模块。
...
- `fongtao-starter-file` 负责启用 x-file-storage；`fongtao-starter-forest`、`fongtao-starter-lock` 仅聚合 Forest、Lock4j 官方 starter；`fongtao-starter-observability` 承载 Actuator、Arthas，并默认关闭 Arthas。
...
- 跨模块树结构优先复用 core 的 `TreeUtil`；通过节点 ID、父 ID 和 `withChildren` 映射构建不可变树，禁止在 Facade 或 Controller 复制分组递归、循环保护和 children 组装逻辑。
...
- 只有 starter 自身真正消费的配置才使用 `@ConfigurationProperties(prefix = "fongtao.<module>")`；第三方组件必须优先使用其官方配置前缀，不得创建不生效的二次映射。
...
- starter 不得携带 `application.yaml` 或偷塞应用级默认配置；需要默认值时使用受控自动配置或官方 properties 默认值。
...
- Controller 只处理请求接入、参数绑定和响应包装，只依赖 Facade；Facade 负责业务规则、认证上下文、跨 Service 编排和写用例事务，不返回 `R<T>`；Service 只负责本对象数据库查询与持久化；Mapper 只负责 SQL 访问。
- Facade 抽象接口使用 `I*Facade` 命名，实现类放在同级 `facade.impl` 子包且不加 `Impl` 后缀；Facade 不直接访问 Mapper。
- 写用例的事务注解放在 Facade 实现类或明确的 Facade 方法；Controller、Mapper 和纯数据库 Service 不声明事务边界。
...
- 权限与基础数据管理按表对象优先拆分：每个主表独立提供对应的 Controller、Facade、Service、Mapper、Converter 和对象专属 DTO；类型名由实际表名转换为 PascalCase，例如 `sys_org -> SysOrg`、`product_order -> ProductOrder`。禁止以 `Rights`、`Basedata`、`Management` 等跨表聚合类型承载管理用例。
- 管理接口根路径使用实际表名的 kebab-case，例如 `sys_org` 对应 `/sys-org`、`product_order` 对应 `/product-order`；所有管理动作使用 `POST`，基础动作统一为 `/page`、`/tree`、`/get-by-id`、`/create`、`/update-by-id`、`/delete-by-id`，主键字段使用对应实体的 ID 字段。
...
- 管理 DTO 按对象放在 `domain.dto` 与 `domain.dto.param`：分页查询为 `*PageParam`，新增、更新为 `*CreateParam`、`*UpdateParam`，出参为 `*Row`；更新参数和 Row 必须包含 `version`，不要继续使用泛化 `*Command`、`*View` 或跨表 DTO。
- `sys_rights_extra` 是 `sys_rights` 的内部子表，只允许由 `SysRights` 的 Facade 和 Service 维护，不新增独立 Controller 或 Facade。
- MyBatis-Plus 全局 ID 策略保持 `ASSIGN_UUID`，持久化实体不得覆盖为 `IdType.INPUT` 或在 Facade/Service 手工生成 UUID；全局更新策略保持 `FieldStrategy.ALWAYS`。

## 安全与日志规则

- 禁止在响应、前端可解码 token 或日志中暴露密码、token、client-secret、Cookie、完整手机号/邮箱、完整请求体、完整 claims 等敏感信息。
- Git 跟踪的应用配置中的数据库密码、缓存密码、JWT secret 等敏感值必须使用 ENC(...)；Jasypt 主口令只能保存在工作区外并受 ACL 保护的本机文件中，通过 JVM 系统属性 jasypt.encryptor.password 注入；使用 Maven spring-boot:run 时必须通过 spring-boot.run.jvmArguments 传给应用 JVM，禁止提交、打印或写入环境变量。
- Security starter 不提供默认 JWT secret，应用必须显式配置；CORS 默认不开放跨域，应用按实际前端来源配置允许地址。
...
- 仅在用户明确要求运行测试或构建验证时，才执行受影响模块 Maven test；跨模块依赖变更时再按要求运行 starter 与业务侧完整构建。
- 除非用户明确要求启动工程、运行测试或执行构建验证，否则默认不执行这些会占用端口、外部服务或较长时间的命令；未执行时应说明验证边界，并优先完成静态检查。
