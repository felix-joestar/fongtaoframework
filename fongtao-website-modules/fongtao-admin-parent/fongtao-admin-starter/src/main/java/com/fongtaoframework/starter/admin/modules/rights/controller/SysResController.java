...
public class SysResController {
    ...
    @PostMapping(RequestPathConstants.PAGE) @PreAuthorize("hasAuthority('admin:sys-res:page')")
    public R<?> page(@RequestBody(required = false) SysResPageParam param) { return R.success(sysResFacade.page(param)); }
    @PostMapping(RequestPathConstants.TREE) @PreAuthorize("hasAuthority('admin:sys-res:tree')")
    public R<List<SysResRow>> tree() { return R.success(sysResFacade.tree()); }
    @PostMapping(RequestPathConstants.GET_BY_ID) @PreAuthorize("hasAuthority('admin:sys-res:get-by-id')")
    public R<SysResRow> getById(@Valid @RequestBody SysResIdParam param) { return R.success(sysResFacade.getById(param.sysResId())); }
    @PostMapping(RequestPathConstants.CREATE) @PreAuthorize("hasAuthority('admin:sys-res:create')")
    public R<Void> create(@Valid @RequestBody SysResCreateParam param) { sysResFacade.create(param); return R.success(); }
    @PostMapping(RequestPathConstants.UPDATE_BY_ID) @PreAuthorize("hasAuthority('admin:sys-res:update-by-id')")
    public R<Void> updateById(@Valid @RequestBody SysResUpdateParam param) { sysResFacade.updateById(param); return R.success(); }
    @PostMapping(RequestPathConstants.DELETE_BY_ID) @PreAuthorize("hasAuthority('admin:sys-res:delete-by-id')")
